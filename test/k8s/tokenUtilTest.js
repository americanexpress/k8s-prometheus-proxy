...
describe('test token utils', () => {
  ...
  it('test get token from properties', () => {
    var readFileStub = sinon.stub(fs, 'readFileSync').returns(Buffer.from("K8S_GLOBAL_TOKEN=mytoken11"));
  ...
  it('test get token raw', () => {
    var readFileStub = sinon.stub(fs, 'readFileSync').returns(Buffer.from("mytoken"));
    const token = tokenUtil.deriveToken('path/to/token');
  ...
  it('test null token', () => {
    var readFileStub = sinon.stub(fs, 'readFileSync').returns(null);
    const token = tokenUtil.deriveToken('path/to/token');
